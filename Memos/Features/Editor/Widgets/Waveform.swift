...
/// The meter trace, drawn as bars. Played bars are solid and the rest are
/// faded, which doubles as the progress indicator so the card needs no
/// separate track.
struct Waveform: View {
    ...
    /// An empty clip still draws a bar row, so the card keeps its shape before
